...
Status: SS-SAFE-001 complete; SS-CUST-001 proposed next, not started.
...
- **D1** — The complete six-file chain under `apps/shop-suit/supabase/migrations/`. Local catalog after applying all six versions: 26 public base tables, 26 public views, 25 public RLS policies, 30 public functions and 19 `shop_private` functions. Business objects are in `public`; helpers remain in `shop_private`.
...
| ID | Short requirement | Priority/type | Status | Repository evidence | Database evidence | UI/workflow evidence | Test evidence | Known gap | Dependencies | Preservation/security note |
|---|---|---|---|---|---|---|---|---|---|---|
| TEN-01 | Isolate all business data by shop tenant | P0 / tenancy and identity | `partially implemented` | E2,E3 | D2,D3,D12 | U1,U3 | T1,T4,T7 | Read policies and every currently supported write command were inspected and hardened; dormant/future command surfaces remain attached to their implementation tasks. | SS-TEAM-001 and each future domain package | Preserve portal/profile/membership separation and cache clearing. |
| TEN-02 | Require active authorized membership for reads and mutations | P0 / tenancy and identity | `implemented` | E2,E3 | D2,D3,D12 | U1,U3 | T1,T4,T7 | Supported commands reject outsiders, unprivileged employees and suspended members; an active employee succeeds only with the existing permission grant. | SS-TEAM-001 for invitation/removal workflows | Preserve the current server-side membership and permission model. |
...
| ID | Short requirement | Priority/type | Status | Repository evidence | Database evidence | UI/workflow evidence | Test evidence | Known gap | Dependencies | Preservation/security note |
|---|---|---|---|---|---|---|---|---|---|---|
| SAFE-01 | Enforce tenant isolation and least privilege on reachable objects | P0 / security and integrity | `partially implemented` | E3,E4,E5,E6,E8,E9,E12 | D2,D3,D5,D6,D7,D8,D9,D11,D12 | U2,U4 | T1,T2,T4,T6,T7 | Every currently reachable write command was reviewed; public wrappers are the only browser write API and direct table/private-command access is denied. Dormant/future objects remain scoped to later packages. | Each future domain package | RLS, grants and RPC authorization remain authoritative. |
| SAFE-02 | Review RLS, grants, definer ownership and cross-shop boundaries | P0 / security and integrity | `partially implemented` | E3,E4,E5,E6,E8,E9,E12 | D2,D3,D5,D6,D7,D8,D9,D11,D12 | U2,U4 | T1,T2,T4,T6,T7 | Supported wrappers now use fixed-path definer execution without browser access to private bodies; relevant direct writes and legacy sale execution remain revoked. Dormant objects were not promoted. | Each future domain package | Re-review each newly exposed command rather than treating schema foundation as proof. |
| SAFE-03 | Require same-shop referenced records | P0 / security and integrity | `partially implemented` | E3,E4,E5,E6,E8,E9,E12 | D3,D5,D6,D7,D12 | U2,U4 | T1,T4,T6,T7 | Supported catalog, stock, expense and purchase references are command-bound to the target shop; product/category/vendor/stock links also have validated composite constraints. Future customer/sale/payment references remain open. | SS-CUST-001 and each future domain package | Cross-shop failures are server-side and leave no partial supported business effect. |
...
| SAFE-08 | Evolve schema and data forward without rewriting history | P0 / security and integrity | `partially implemented` | E11 | D1,D11,D12 | U4 | T6,T7 | SS-SAFE-001 is one additive forward migration; it validates existing rows and fails rather than repairing mismatches, with no history rewrite or remote application. | Preserve in every later migration | Existing IDs, FIFO layers, documents and audit records remain unchanged. |
...
| SAFE-10 | Keep history queryable after archival, staff or subscription changes | P0 / security and integrity | `partially implemented` | E3,E4,E5,E6,E8,E9,E12 | D2,D3,D5,D6,D7,D9,D11,D12 | U2,U4 | T1,T4,T6,T7 | Catalog archives, expense voids, purchase reversals and reads after subscription expiry preserve history; staff-removal and future integration history need later workflow proof. | SS-TEAM-001 and future integration decision | No supported history was deleted or rewritten. |
...
| VAL-09 | Verify roles, suspended members, outsiders and cross-shop isolation | Validation / evidence | `implemented` | E3,E11,E12 | D2,D3,D12 | U1,U2,U4 | T1,T6,T7 | Focused rollback fixtures cover owner success, authorized/unauthorized employee behavior, suspended membership, outsider denial and materially distinct cross-shop reference paths. | Re-run per future command surface | No hosted database or provider change was made. |
...
Dependency justification: the currently supported mutation boundary is hardened. Customer master data is the next bounded prerequisite before sale issuance and payments can be exposed safely.

Do not start this task automatically. It must define and test its own same-shop references, authorization, history and future receivable contract without reusing the dormant legacy sale finalizer.

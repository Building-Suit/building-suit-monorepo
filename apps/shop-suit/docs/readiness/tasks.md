...
Status: SS-SALE-001 complete; SS-PAY-001 proposed next, not started.
...
Effective local inventory after all eight migrations:

- 28 public tables: tenancy/profile/roles, plans/subscriptions, products/services, clients/vendors, sales/purchases, FIFO inventory, payments, expenses/periods, stock requests, sale requests and sale number counters.
...
- Customer master data and product/service/mixed sale issuance are supported connected workflows. Payments/team/reports retain substantial schema foundation but no supported connected end-to-end workflow. The broken legacy sale finalizer remains revoked and unused.
...
- **D1** — The complete eight-file chain under `apps/shop-suit/supabase/migrations/`. Local catalog after applying all eight versions: 28 public base tables, 26 public views, 25 public RLS policies, 41 public functions and 34 `shop_private` functions. Business objects are in `public`; helpers remain in `shop_private`.
...
- **U4** — No connected UI exists for customer receipts, supplier payments, team management or reports; sales are covered by U7.
...
| SALE-09 | Allow optional customer for ordinary cash sale where permitted; require customer where credit/outstanding balance needs one | P0 / sales | `partially implemented` | E14 | D14 | U7 | T9 | Draft customer is optional, but issue requires a same-shop active customer because no atomic settled cash payment exists. Customerless finalized cash sale remains deferred. | SS-PAY-001 | No anonymous outstanding receivable or fake paid state. |
...
| SALE-13 | Define correction/void/refund/return behavior without silently rewriting finalized history | P0 / sales | `missing` | E14 | D14 | U7 | T9 | Issued immutability and stable source IDs provide a foundation, but no correction/void/refund/return command is approved or exposed. | SS-SALE-002; SS-PAY-001 | Never rewrite issued history silently. |
...
| SAFE-03 | Referenced customer/supplier/product/service/payment/role/category/document/stock records belong to the same shop where applicable | P0 / security and integrity | `partially implemented` | E3,E4,E5,E6,E8,E9,E12,E13,E14 | D3,D5,D6,D7,D12,D13,D14 | U2,U6,U7 | T1,T6,T7,T8,T9 | Supported sale/customer/catalog/stock/expense/purchase references are shop-bound and cross-shop sale failures leave no partial effect; payment commands remain future scope. | Each future domain package | Tenant identity is enforced server-side. |
| SAFE-04 | Protect finalized documents and stock/payment history from traceability-destroying silent mutation | P0 / security and integrity | `partially implemented` | E8,E13,E14 | D5,D7,D9,D13,D14 | U2,U6,U7 | T7,T8,T9 | Issued sales/lines and inventory movements are immutable and purchase/customer history is preserved; payment correction/reversal policy remains open. | SS-PAY-001; SS-SALE-002 | UI controls are not the integrity boundary. |
| SAFE-05 | Use idempotency for money-, stock-, sale-, and purchase-affecting mutations that may retry | P0 / security and integrity | `partially implemented` | E3,E6,E8,E9,E14 | D5,D6,D7,D12,D14 | U2,U7 | T1,T7,T9 | Sale draft/issue, manual stock, expense, supplier purchase and bootstrap retries are protected; payment idempotency remains deferred. | SS-PAY-001 | Preserve request identity and conflict detection. |
| SAFE-06 | Concurrency must not allow quota overrun, overselling beyond policy, duplicate numbering, duplicate payment application, or duplicate stock movement | P0 / security and integrity | `partially implemented` | E3,E4,E5,E6,E8,E9,E14 | D3,D4,D5,D6,D7,D12,D14 | U2,U7 | T1,T7,T9 | Real parallel sale fixtures prove no oversell, duplicate movement or duplicate number; payment allocation concurrency remains deferred. | SS-PAY-001 | Product and counter locks use deterministic supported paths. |
...
| SAFE-08 | Do not casually rewrite applied migration history or existing customer data; use additive/backward-compatible migration strategy where practical | P0 / security and integrity | `partially implemented` | E11,E13,E14 | D1,D11,D12,D13,D14 | U6,U7 | T6,T7,T8,T9 | SS-SALE-001 is one forward migration that preserves existing IDs/documents/FIFO layers and adds nullable snapshots before supported population; no remote migration was applied. | Preserve in every later migration | Historical migrations and customer data were not rewritten. |
...
| VAL-04 | Test retry/double-submit so sale, purchase, payment, expense, or stock movement is not duplicated | Validation / evidence | `partially implemented` | E6,E8,E9,E11,E14 | D5,D6,D7,D12,D14 | U1,U2,U7 | T1,T7,T9 | Sale, stock, expense and purchase retries/conflicts are covered; payment retry cannot be tested before SS-PAY-001. | SS-PAY-001 | Preserve identical retry success and conflicting-key failure. |
...
| VAL-09 | Test owner/staff permissions, suspended membership, outsider access, and cross-shop isolation | Validation / evidence | `implemented` | E3,E11,E12,E13,E14 | D2,D3,D12,D13,D14 | U1,U2,U6,U7 | T1,T6,T7,T8,T9 | Sale coverage adds owner/delegated success, unprivileged/suspended/outsider denial and foreign customer/product/service rejection to existing paths. | Re-run per future command surface | No hosted database or provider change was made. |
...
| VAL-11 | Test Arabic/English, RTL/LTR, responsive layout, and important empty/error/loading states for changed flows | Validation / evidence | `partially implemented` | E11,E12,E13,E14 | D1,D11,D13,D14 | U1,U2,U6,U7 | T3,T8,T9 | Localized responsive sale/customer states are implemented and type/lint checked; authenticated manual browser verification remains unavailable. | SS-UX-001 and SS-VAL-001 | Verify sale composition/detail in Arabic/English, RTL/LTR and mobile/desktop. |

...
| 02 — Database target and contract | Complete (schema-only recovery) | Confirm `shop_crm`, capture a schema-only recovery baseline, reconcile historical migrations | Target, schema replay probe, migration guard and app-to-cloud map documented; full data restore belongs to Task 13 |
...
| 04 — Supabase-client-only app | Partial: shell, pricing, shop selection, dashboard, Products, Services, Inventory and Expenses repaired; missing feature pages pending | Remove API layer, map shop selection and pricing to `shop_crm`, replace legacy page contracts and types | Build and typecheck pass; authenticated browser journeys remain pending |
...
| 08 — Catalog and counterparties | Partial: owner products and services working; customers/vendors pending | Align products/services/customers/vendors; archive/search; quotas | Product and service fixtures and build pass; remaining catalogs and browser journeys pending |
| 09 — Purchases and inventory | Partial: manual Pro stock adjustments complete; supplier posting pending | Atomic supplier posting, FIFO batches, stock adjustments/write-offs | Manual fixture passes; supplier posting, sale integration and browser journey pending |
...
Task 03's rollback-only fixture passed on the confirmed hosted project. Task 04c
exposed `shop_crm` through a PostgREST role setting and verified anonymous plan
and portal reads; authenticated owner/employee journeys remain unverified. The
next bounded step is supplier posting and sales. Do not grant direct browser
writes as a shortcut: each later write

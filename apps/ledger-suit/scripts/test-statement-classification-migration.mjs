// Rehearse only on the explicitly owned, disposable classification-review backend.
...
assert.equal(process.env.LEDGER_CLASSIFICATION_DISPOSABLE_TEST, '1', 'Explicit disposable rehearsal opt-in required')
...
const migration = '20260919105319_dated_statement_classification.sql'
...
const fingerprint = () => sql(`select jsonb_build_object(
 'accounts',(select jsonb_agg(to_jsonb(a) order by id) from public.accounts a),
 ...
 'balances',(select jsonb_agg(to_jsonb(b) order by account_id) from public.account_balances b));`)
...
assert.equal(sql('select count(*) from public.account_statement_classifications'), '0', 'No inferred classification backfill')
const data = JSON.parse(before)
console.log(JSON.stringify({ result: 'PASS', accounts: data.accounts.length, transactions: data.transactions.length, entries: data.entries.length, sha256: createHash('sha256').update(before).digest('hex'), reportUnchanged: true, noClassificationBackfill: true }))

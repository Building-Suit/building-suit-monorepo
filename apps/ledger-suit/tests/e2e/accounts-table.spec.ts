...
test('real create/edit/archive refresh the table and existing transaction selector without reload', async ({ page }) => {
  ...
  const accountTable = page.getByRole('tabpanel').getByRole('table')
  await expect(accountTable).toContainText(name)
  await accountTable.getByRole('button', { name: 'Edit', exact: true }).click()
  await page.getByLabel('Account name', { exact: true }).fill(`${name} updated`)
  await page.getByRole('button', { name: 'Save', exact: true }).click()
  await expect(accountTable).toContainText(`${name} updated`)
  await page.getByRole('link', { name: 'Transactions', exact: true }).first().click()
  await page.getByRole('button', { name: 'Expense', exact: true }).click()
  await page.getByRole('button', { name: 'New transaction', exact: true }).first().click()
  ...
  await accountTable.getByRole('button', { name: 'Archive', exact: true }).click()
  await expect(accountTable).not.toContainText(name)
  await page.getByLabel('Show archived').check()
  await expect(accountTable).toContainText(name)

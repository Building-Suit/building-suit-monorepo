...
test('landing page explains the product and leads to a cardless trial', async ({ page }) => {
  ...
  await expect(page.getByTestId('payment-method-branding')).toHaveText('Secure payments powered by Paymob')
...
test('signup verifies email by OTP before provisioning the free trial', async ({ page }) => {
  ...
  const consent = page.getByLabel(/I agree to the Terms & Conditions and Refund & Cancellation Policy/)
  const createAccount = page.getByRole('button', { name: 'Create account and start free trial' })
  await expect(page.getByRole('link', { name: 'Terms & Conditions' })).toHaveAttribute('href', '/terms')
  await expect(page.getByRole('link', { name: 'Refund & Cancellation Policy' })).toHaveAttribute('href', '/refund-cancellation')
...
test('signup continues to its saved organization after the OTP tab is closed', async ({ page }) => {
  ...
  await page.getByLabel(/I agree to the Terms & Conditions and Refund & Cancellation Policy/).check()
...
test('billing stays mounted and product navigation remains client-side', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Journal Center', level: 1 })).toBeVisible()

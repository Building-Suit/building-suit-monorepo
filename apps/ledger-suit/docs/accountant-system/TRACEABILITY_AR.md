...
تحديث V2-IMP-015 في 2026-09-26: [سجل القبول](v2-acceptance/README.md) يربط COA-09 وCORE-08 وFS-08 وJRN-02/03 وVAL-01–08 بالأدلة الفعلية والعوائق، و[سجل المتطلبات](v2-acceptance/requirements.json) يفصل التنفيذ والاختبار والنشر وقبول المحاسب لكل المتطلبات الـ١٣٨. الحالة BLOCKED؛ لا نجاح SQL/متصفح/استعادة أو قبول محاسب مثبت. V2-D03 معتمد لكن التنفيذ المتسلسل يحتاج مهمة إصلاح مستقلة؛ لا تُغيّر هذه المراجعة سلوك التطبيق.

تحديث V2-IMP-013 في 2026-09-26: TAX-01–07 منفذة محليًا ضمن مصر/EGP/التسجيل الصريح/التوريد المحلي القياسي ١٤٪ فقط عبر إعداد مؤرخ، مستندات ضريبية ثابتة، محرك الترحيل المشترك، مراقبة مخرجات ومدخلات محمية، وإشعارات/عكس مرتبط، وتقرير source=VAT=GL. VAL-05 يملك fixture SQL ووحدة دقيقة وfixture واجهة EN/AR/RTL؛ نجحت الوحدة وlint وtypecheck وbuild والحدود/التوكنات وJSON وdiff، بينما تعذر SQL بسبب Docker والمتصفح بسبب منع المنفذ. لا نشر أو قاعدة مستضافة أو UAT، ولا ادعاء امتثال أو تكامل حكومي. [سجل الضريبة](../tax-vat-accounting.md).

تحديث V2-IMP-012 في 2026-09-26: DIM-01–07 منفذة محليًا وفق V2-D10 عبر قيم مراكز تكلفة/مشروعات مضبوطة، وسياسات اختيارية/مطلوبة لكل حساب، وتوزيعات بند دقيقة وثابتة بعد الترحيل، وتقارير GL/TB/P&L/BS/CF بالتصفية/التجميع و«غير معيّن» الصريح. VAL-05 يملك fixture SQL ووحدة دقيقة وfixture واجهة ثنائية اللغة، لكن SQL والمتصفح الأصليين لم يُشغلا بسبب Docker/telemetry/منع المنفذ؛ نجحت الوحدة الكاملة وlint وtypecheck وbuild وفحص الحدود وJSON وdiff. لا UAT أو نشر أو قاعدة مستضافة. التفاصيل في [سجل الأبعاد المحاسبية](../accounting-dimensions.md).

تحديث V2-IMP-011 في 2026-09-25: FA-01–09 منفذة محليًا وفق V2-D09 عبر سجل صريح مرتبط بقيد الاقتناء، ونسخ سياسة وجداول يومية، وترحيل إهلاك متسلسل، واضمحلال واستبعاد وربح/خسارة، ومطابقة مؤرخة مع الأستاذ، وعكس/استبدال مرتبط، وصفحة `/fixed-assets` باللغتين. نجحت الوحدة الكاملة وlint وtypecheck وbuild وفحص الحدود وJSON وdiff؛ تشغيل SQL/التزامن محجوب بصلاحية Docker والمتصفح محجوب بمنع فتح المنفذ. لا UAT أو نشر أو قاعدة مستضافة. التفاصيل في [سجل الأصول الثابتة](../fixed-assets.md).

تحديث V2-IMP-009 في 2026-09-25: AP-01–09 منفذة محليًا وفق V2-D06 عبر `ap_documents` و`ap_allocations` ومحرك الترحيل المشترك وصفحة `/payables`؛ COA-09 وVAL-04 يملكان الآن عقد مزود AP واختبارات مركزة، لكن تشغيل SQL/التزامن والمتصفح الأصلي بقي غير منفذ بسبب قيود Docker/الشبكة/المنفذ المحلي. نجحت وحدة دقة المبالغ وlint وtypecheck وفحص JSON وdiff. لا UAT أو نشر أو قاعدة مستضافة. التفاصيل في [سجل دفتر الموردين](../supplier-subledger.md).
...
## AS-UX-03 — الوسوم والمساعدة الدائمة ووضوح الشجرة، 2026-09-23

PASS: 49 unit و9 browser وdiff-check/lint/typecheck/build. حالة متصفح فعلية تنشئ وسمًا وتربطه بمعاملة وتفلتر به بعد إعادة التحميل ثم تزيله. حالات مستقلة تثبت بقاء عنصر المساعدة نفسه عند التنقل، واستعادة التركيز، وموقعه فوق تنقل الهاتف، وظهور أقسام الشجرة بالإنجليزية والعربية وRTL. انحدار AS-UX-01 يثبت بقاء كشف الحساب وإنشاء الفرع والفلاتر والصلاحيات. لا SQL في الحزمة؛ طُبقت الترحيلات المعلقة على الخلفية المحلية فقط قبل التحقق. [الحزمة](work-packages/AS-UX-03.md) و[الدليل](../evidence/as-ux-03/README.md). لا تغيير لقبول ق٠١–ق٢٤ أو ادعاء UAT/نشر.

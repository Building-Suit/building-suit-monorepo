...
select is(has_function_privilege('anon','public.create_account(uuid,text,public.account_type,public.account_subtype,character,text,uuid,public.normal_balance,uuid)','execute'), false, 'anonymous callers cannot create classified accounts');

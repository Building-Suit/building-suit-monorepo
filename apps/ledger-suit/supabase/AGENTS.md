# ledger-suit database development

Read the root and owning app rules, `docs/shared/database.md`, the environment registry and relevant product contracts.

- This is the app-owned Supabase directory; run `pnpm db ledger-suit <command>` from the monorepo root. Production and staging share this product’s versioned history, with separate refs, secrets and Auth users.
